...
class TestActionWeights:
    ...
    @pytest.mark.parametrize("action, expected_score", [
        ("get",    0),
        ("list",   0),
        ("watch",  0),
        ("create", 1),
...
class TestResourceWeights:
    ...
    def test_sensitive_resource_adds_one(self, resource: str) -> None:
        ...
        assert rs.score == 1, (
            f"Expected 1 for resource={resource!r}, got {rs.score}"
        )
...
class TestParamsInspection:
    ...
    def test_privileged_true_adds_three(self) -> None:
        rs = score("create", "deployments", "demo",
    ...
    def test_host_pid_adds_two(self) -> None:
        rs = score("create", "pods", "demo",
                   params={"hostPID": True, "image": "nginx:alpine"})
        assert rs.score >= 3
        assert any("hostPID" in r for r in rs.reasons)

    def test_untrusted_image_adds_one(self) -> None:
        rs_trusted   = score("create", "deployments", "demo",
                              params={"image": "nginx:alpine"})
        rs_untrusted = score("create", "deployments", "demo",
                              params={"image": "cryptominer:latest"})
    ...
    def test_trusted_image_adds_zero(self) -> None:
        for image in ["nginx:alpine", "python:3.11", "gcr.io/myco/app:v1"]:
            ...
            assert rs.score == 1, f"Expected score 1 for trusted image {image!r}, got {rs.score}"

    def test_high_replicas_adds_one(self) -> None:
        rs_normal = score("create", "deployments", "demo",
                           params={"image": "nginx:alpine", "replicas": 3})
        rs_high   = score("create", "deployments", "demo",
